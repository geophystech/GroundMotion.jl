...
function gmpe_corr(config::Params_gmpe_corr,grid::Array{Point_pga_out}, intensity_measures::Array{Point_felt_report};intensity_in_pga::Bool=true, pga_out::Bool=true)
  # ! Convert to ssi felt reports !
  if intensity_in_pga
    intensity_measures = convert_from_pga_to_ssi(intensity_measures, alpha=config.alpha, beta=config.beta)
  end
  ...
  closes_points_index = Array{Tuple{Int, LatLon, Float64, Array{Float64}}}(undef,0)
  for (index, i) in enumerate(intensity_measures)
    ...
    for j in grid
      ...
      distance_to_point = euclidean_distance(current_point, grid_point)/1000
  ...
  for (index_i, i) in enumerate(intensity_measures)
    ...
    for (index_j, j) in enumerate(closes_points_index[index_i][4]) # [3] stand for distances array
      if j <= config.h_max
        SIGMA_corr[index_j] = config.sigma_gmpe*(1 - exp(-j/config.h_0))
      end
    end
    if i.is_station
    ...
    numerator_sum = i.intensity - closes_points_index[index_i][3] .+ [x.ssi for x in grid]
    SIGMA_numerator = SIGMA_numerator .+ numerator_sum ./ SIGMA_corr
  end
  # Final action
  I_CORR = ([x.ssi for x in grid] ./ config.sigma_gmpe .+ SIGMA_numerator) ./ (1/config.sigma_gmpe .+ SIGMA_denominator)
  # Form output
